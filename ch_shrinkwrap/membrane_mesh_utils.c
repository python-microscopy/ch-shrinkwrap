                             ...
                             points_t *dEdN)
{
    ...
    for (i=0;i<n_vertices;++i)
    {
        ...
        for(j=0;j<n_neighbors;++j)
        {
            ...
            dE_neighbors[i] += ((PRECISION)((dAj*w*0.5*((double)kc)*SQUARE(2.0*kj_1-((double)c0)) - Aj*w*0.5*((double)kc)*SQUARE(2.0*kj-((double)c0)))))/dN;  // eV, note this only on mean curvature
