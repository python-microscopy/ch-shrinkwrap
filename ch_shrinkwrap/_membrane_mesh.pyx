...
cdef class MembraneMesh(TriangleMesh):
    ...
    def hole_candidate_faces(self, points, eps=10.0):
        """
        Find all mesh faces that have no points within a distance eps. Return
        the index of these faces.
        """
        ...
        return inds[dist>eps]

    def pair_candidate_faces(self, candidates):
        """
        Find opposing candidate faces that can reach each other by traveling 
        along their negative normals.
        """
        from ch_shrinkwrap import util

        paired = -1*np.ones_like(candidates, dtype=int)
        ...
        # TODO: Rather than skipping ones that are assigned, should we check
        # all assignable candidates and choose the one that is "best"?
        for i, ci in enumerate(candidates[:-1]):
            # If this face has been paired, do not pair again
            if paired[i] >= 0:
                continue
            nci = self._faces['normal'][ci]
            
            # Upper triangle, please
            for j, cj in enumerate(candidates[(i+1):]):
                jj = j+i+1
                if paired[jj] >= 0:
                    continue

                # Don't pair faces that are obviously not facing away 
                # from one another
                ncj = self._faces['normal'][cj]
                if (nci*ncj).sum() > 0:
                    continue

                # Now test that the faces are reachable. Is the projection of 
                # the center of ci onto the plane spanned by cj inside of cj?
                Pcj = I - ncj[:,None]*ncj[None,:]
                pci = Pcj @ face_centers[i,...]
                if not util.point_inside_triangle(pci, fv_pos[jj,...]):
                    #continue
                    # Is the projection of the center of cj onto the plane spanned
                    # by ci inside ci?
                    Pci = I - nci[:,None]*nci[None,:]
                    pcj = Pci @ face_centers[jj,...]
                    if not util.point_inside_triangle(pcj, fv_pos[i,...]):
                        continue
                        
                # These faces are reachable along their negative normals
                paired[i], paired[jj] = jj, i
                
                # i is now paired
                break
                
        kept_inds = (paired>=0)
        new_paired_inds = np.cumsum(kept_inds)-1

        return candidates[kept_inds], new_paired_inds[paired[kept_inds]] #, candidates, paired
    ...
    def empty_prism_candidate_faces(self, points, candidates, candidate_pair, eps=10.0):
        ...
        for i, ci in enumerate(candidates):
            ...
            kept_cands[i] = np.sum(below_hp0_ci & below_hp1_ci & below_hp2_ci \
                                & below_hp0_cj & below_hp1_cj & below_hp2_cj) == 0
        
        new_paired_inds = np.cumsum(kept_cands)-1
        
        return candidates[kept_cands], new_paired_inds[candidate_pair[kept_cands]]

    def connect_candidates(self, candidates):
        """
        Compute the connected component labeling of the kept faces 
        such that faces that share vertices are considered connected.
        """
        he = self._faces['halfedge'][candidates]
        v0 = self._halfedges['vertex'][self._halfedges['prev'][he]]
        v1 = self._halfedges['vertex'][he]
        ...
        component = -1*np.ones_like(candidates, dtype=int)
        curr_component = 0
        for i in range(len(v0)-1):
            for j in range(i+1,len(v0)):
                fvi, fvj = fv[i,:], fv[j,:]
                if np.any(fvi[:,None] == fvj[None,:]):
                    c = np.minimum(component[i], component[j])
                    if c == -1:
                        c = curr_component
                        curr_component += 1
                    component[i], component[j] = c, c
    ...
    def update_topology(self, candidates, candidate_pairs, component, euler):
        ...
        for i, c in enumerate(unique_components):
            ...
            print(f"Component {c} has Euler characteristic {euler[i]}.")
            if euler[i] == 0:
    ...
    def cut_and_punch(self, pts, eps=10.0):
        hc = self.hole_candidate_faces(pts, eps=eps)
        cands, pairs = self.pair_candidate_faces(hc)
        empty_cands, empty_pairs = self.empty_prism_candidate_faces(pts, cands, pairs, eps=eps)
        i = self.connect_candidates(empty_cands)
        chi = self.connected_candidates_euler_characteristic(empty_cands, i)

        self.update_topology(empty_cands, empty_pairs, i, chi)
    ...
    def opt_conjugate_gradient(self, points, sigma, max_iter=10, step_size=1.0, **kwargs):
        ...
        for j in range(max_iter//rf):
            ...
            # Cut and hole punch
            if dr and ((((j+1)*rf) % self.delaunay_remesh_frequency) == 0):
                self.cut_and_punch(points, eps=self.delaunay_eps)
